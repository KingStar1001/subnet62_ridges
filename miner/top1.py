...
def agent_main(input_dict: Dict[str, Any], repo_dir: str = "repo", test_mode: bool = False):
    ...
    logger.info(f"[AGENT_MAIN] Starting with run_id: {os.getenv('RUN_ID', '')}")
    ...
    try:
        ...
        logger.info(f"[AGENT_MAIN] Problem type detected: {problem_type}")

        if problem_type == PROBLEM_TYPE_FIX:
            logger.info("[AGENT_MAIN] Starting FIX workflow")
            result = process_fix_task(input_dict)
        else:
            logger.info("[AGENT_MAIN] Starting CREATE workflow")
            result = process_create_task(input_dict)
            
        logger.info(f"[AGENT_MAIN] Workflow completed, result length: {len(str(result))}")
    except Exception as e:
        logger.error(f"[AGENT_MAIN] Exception: {e}, falling back to FIX workflow")
        result = process_fix_task(input_dict)

    os.system("git reset --hard")
    logger.info("[AGENT_MAIN] Completed")
    return result

def check_problem_type(problem_statement: str) -> str:
    logger.info("[CHECK_PROBLEM_TYPE] Analyzing problem type")
    retry = 0
    while retry < 10:
        try:
            messages = [
            ...
            if response not in [PROBLEM_TYPE_CREATE, PROBLEM_TYPE_FIX]:
                logger.warning(f"[CHECK_PROBLEM_TYPE] Invalid response: {response}, retrying...")
                retry += 1
            else:
                logger.info(f"[CHECK_PROBLEM_TYPE] Determined type: {response}")
                break
        except Exception as e:
            logger.error(f"[CHECK_PROBLEM_TYPE] Error: {e}")
            retry += 1
...
def generate_solution_with_multi_step_reasoning(problem_statement: str, code_skeleton: str) -> str:
    logger.info("[MULTI_STEP] Starting multi-step reasoning workflow")
    retry = 0
    ...
    while retry < 10:
        try:
            ...
            logger.info(f"[MULTI_STEP] Code generation completed ({len(code_response)} chars)")
            
            # STEP 2: Infinite Loop Check
            loop_check_messages = [
                {
                    "role": "system",
            ...
            if lines[0].endswith(".py") == False:
                logger.warning(f"[MULTI_STEP] Format validation failed: '{first_line}' - retrying")
                retry += 1
            ...
            logger.error(f"[MULTI_STEP] Exception: {e}")
            retry += 1
            if retry < 10:
                time.sleep(2)
...
def generate_initial_solution(problem_statement: str, code_skeleton: str) -> str:
    logger.info("[GENERATE_SOLUTION] Starting solution generation")
    retry = 0
    ...
    while retry < 10:
        try:
            ...
            if solution:
                ...
                response = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME)
                logger.info(f"[GENERATE_SOLUTION] Fallback completed ({len(response)} chars)")
...
def process_create_task(input_dict):
    ...
    logger.info(f"[CREATE_TASK] Problem statement processed ({len(problem_statement)} chars)")
    print(problem_statement)

    # Generate code skeleton
    code_skeleton = get_code_skeleton()
    logger.info(f"[CREATE_TASK] Code skeleton generated ({len(code_skeleton)} chars)")
